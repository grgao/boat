...
# Set up PWM for sine wave generation
sine_pwm = GPIO.PWM(SINE_PIN, 1000)  # 1 kHz base frequency
...
def generateSineWave():
    for i in range(STEPS):
        ...
        sine_pwm.ChangeDutyCycle(duty)
        time.sleep(1 / (SINE_FREQ * STEPS))  # Adjust step timing

try:
    while True:
        #continuously generate te sine wave
        generateSineWave()
